...
def test_pump_should_not_corrupt_suri_keys(tmp_path, monkeypatch):
    ...
    # 2. Act: 添加新函数并运行 pump
    (project_root / rel_path).write_text(
    ...
    with sig_path.open("r") as f:
        ...
        # 我们期望这里依然全是 SURI
        assert all(k.startswith("py://") for k in keys), (
            f"Pump corrupted SURI keys! Found: {keys}"
        )
...
def test_check_relink_should_upgrade_legacy_signatures(tmp_path, monkeypatch):
    ...
    with sig_path.open("r") as f:
        ...
        assert any(k.startswith("py://") for k in keys), (
            f"Check failed to upgrade legacy keys! Found: {keys}"
        )
