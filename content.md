## [WIP] debug: 在 CheckRunner 中添加调试日志

### 用户需求

诊断集成测试中出现的“结果重复”和“结果丢失”问题。

### 评论

我们遇到了令人困惑的测试失败。通过在 `CheckRunner` 中添加临时的打印语句，我们可以直接观察 `AnalysisEngine` 返回了什么，以及 `_translate_results` 做了什么。这是最快定位问题的手段。

### 目标

修改 `stitcher/app/runners/check/runner.py`，在 `analyze_paths` 和 `_translate_results` 中添加打印语句。

### 标签

#intent/debug #flow/ready #priority/high #comp/application #task/action/debug
...
请运行失败的测试，并向我展示 stdout 中的 `DEBUG` 输出。这将揭示真理。
